...
uname -a
ps -ef
To complete the questions below, navigate to http://10.10.54.213:82/ and exploit the cowsay server.
...
Navigate to http://10.10.54.213:85 and get into joseph's account. This application also has a design flaw in its password reset mechanism. Can you figure out the weakness in the proposed design and how to abuse it?
...
Navigate to http://10.10.54.213:86 and try to exploit the security misconfiguration to read the application's source code.
...
It sounds pretty simple! Let's walk through the process an attacker would have to follow in an example scenario. Navigate to http://10.10.54.213:8089/ and follow the instructions in the questions below.
